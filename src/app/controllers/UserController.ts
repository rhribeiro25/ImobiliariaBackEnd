...
routerUnsafe.post('/create', async (req, res) => {
  try {
    ...
    if (userExists) return res.status(400).send(new Error('Email já cadastrado no sistema!'));
...
routerAuth.get('/list', async (req, res) => {
  try {
    ...
    if (!users) return res.status(404).send(new Error('Usuários não encontrados!'));
...
routerAuth.get('/show/:id', async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).send(new Error('Usuário não encontrado!'));
...
routerAuth.patch('/update/:id', async (req, res) => {
  try {
    const user = req.body;
    const updated = await userService.findByIdAndUpdate(req.params.id, user, {
      new: true,
      runValidators: true,
    });
    if (!updated) return res.status(404).send(new Error('Usuário não encontrado!'));
...
routerUnsafe.post('/authenticate', async (req, res) => {
  try {
    ...
    const user: UserInterface = await userService.findByMail(email, '+pass');
    if (!user) return res.status(404).send(new Error('Usuário não encontrado!'));
    const isAuthenticated = await userService.isAuthenticated(pass, user.pass);
    if (!isAuthenticated) return res.status(404).send(new Error('Senha Inválida!'));
    user.pass = '';
    res.status(200).send({ user, token: userService.generateToken(process.env.SECRET, { id: user.id }) });
  } catch (error) {
    return res.status(500).send({
      error: {
        name: error.name,
...
routerUnsafe.post('/forgot_password', async (req, res) => {
  try {
    const { email } = req.body;
    const user: UserInterface = await userService.findByMail(email, '');
    if (!user) return res.status(404).send(new Error('Usuário não encontrado!'));
...
routerUnsafe.patch('/reset_password', async (req, res) => {
  try {
    ...
    const user: UserInterface = await userService.findByMail(email, '+resetToken resetExpires');
    if (!user) return res.status(404).send(new Error('Usuário não encontrado!'));
    if (token !== user.resetToken)
      return res.status(404).send({
        error: { message: 'Token inválido, favor tentar novamente!' },
      });
    if (user.resetExpires && new Date() > user.resetExpires)
      return res.status(404).send({
        error: { message: 'Token expirado, favor gerar token novamente!' },
      });
...
export default [routerUnsafe, routerAuth];
