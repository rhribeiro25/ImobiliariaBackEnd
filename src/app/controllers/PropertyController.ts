...
routerAuth.post('/create', async (req, res) => {
  try {
    ...
    const property = await propertyService.create(newProperty, process.env.USER_ID);
...
routerAuth.get('/list', async (req, res) => {
  try {
    ...
    if (!properties) return res.status(400).send(new Error('Imóvel não encontrado!'));
...
routerAuth.get('/show/:id', async (req, res) => {
  try {
    ...
    if (!property) return res.status(400).send(new Error('Imóvel não encontrado!'));
...
routerAuth.patch('/update/:id', async (req, res) => {
  try {
    ...
    const updatedProperty = await propertyService.findByIdAndUpdate(req.params.id, property, {
      new: true,
      runValidators: true,
    });
    if (!updatedProperty) return res.status(400).send(new Error('Imóvel não encontrado!'));
